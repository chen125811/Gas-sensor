...
# 设置输出目录
output_dir = "F:\\VS code\\code for gas adsorption\\C6H6\\各种树模型\\lightgbm_shap_analysis"
os.makedirs(output_dir, exist_ok=True)

# 设置文件路径
data_path = "F:\\VS code\\code for gas adsorption\\gas adsorption-C6H6.xlsx"
model_params_path = "F:\\VS code\\code for gas adsorption\\C6H6\\各种树模型\\tree_models_results\\best_model_parameters.json"
...
try:
    ...
    df = df.rename(columns={
        ...
        "Amino": "Amino", 
        "carboxyl": "Carboxyl",
        "Gas concentration (kPa)": "Gas_concentration", 
    ...
    # 如果数据集太大，可以选择一个子集进行SHAP分析
    if len(X_test) > 100:
        # 随机选择100个样本
        sample_indices = np.random.choice(len(X_test), 100, replace=False)
        X_sample = X_scaled_df.iloc[sample_indices]  # 使用DataFrame而不是numpy数组
    else:
        X_sample = X_scaled_df  # 使用完整的DataFrame
    ...
    # 11. 计算BET比表面积和孔容的累积贡献度
    bet_pct = shap_importance_df[shap_importance_df['Feature'] == 'BET_surface_area']['SHAP_Importance_Pct'].values[0]
    pore_pct = shap_importance_df[shap_importance_df['Feature'] == 'Pore_volume']['SHAP_Importance_Pct'].values[0]
    combined_pct = bet_pct + pore_pct
    
    print(f"\nBET比表面积SHAP重要性占比: {bet_pct:.2f}%")
    print(f"孔容SHAP重要性占比: {pore_pct:.2f}%")
    print(f"BET比表面积和孔容的累积贡献度: {combined_pct:.2f}%")
    
    # 12. 可视化SHAP重要性占比
    plt.figure(figsize=(12, 8))
    colors = ['#d62728' if feat in ['BET_surface_area', 'Pore_volume'] 
              else '#1f77b4' for feat in shap_importance_df['Feature']]
    sns.barplot(x='SHAP_Importance_Pct', y='Feature', data=shap_importance_df, palette=colors)
    plt.title('Feature Importance by SHAP Values (%)', fontsize=22)
    plt.xlabel('SHAP Importance (%)', fontsize=18)
    plt.ylabel('Feature', fontsize=18)
    plt.grid(axis='x', linestyle='--', alpha=0.7)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "shap_importance_percentage.png"), dpi=300, bbox_inches='tight')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "shap_summary_dot.png"), dpi=300, bbox_inches='tight')
    plt.close()
    ...
    with open(os.path.join(output_dir, "shap_importance_report.md"), 'w', encoding='utf-8') as f:
        ...
        f.write("\n## BET比表面积和孔容的重要性\n")
        f.write(f"- BET比表面积: **{bet_pct:.2f}%**\n")
        f.write(f"- 孔容: **{pore_pct:.2f}%**\n")
        f.write(f"- 累积贡献度: **{combined_pct:.2f}%**\n\n")
        
        f.write("## 结论\n\n")
        
        # 确定最重要的3个特征
        top_features = shap_importance_df.head(3)['Feature'].tolist()
        f.write("基于SHAP值分析，以下特征对C6H6吸附贡献最大：\n\n")
        ...
        # 添加关于BET和孔容的结论
        if 'BET_surface_area' in top_features or 'Pore_volume' in top_features:
            f.write(f"\nBET比表面积和孔容共同贡献了模型预测能力的 **{combined_pct:.2f}%**，")
            if combined_pct > 50:
                f.write("表明它们是决定C6H6吸附性能的关键因素。\n")
            else:
    ...
    # 16. 保存SHAP值显著性分析结果
    shap_significance_df.to_csv(os.path.join(output_dir, "shap_significance_analysis.csv"), index=False)
    ...
    try:
        ...
        if len(X_sample) > 50:
            significance_sample = X_sample.sample(50, random_state=42)
        else:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "shap_significance.png"), dpi=300, bbox_inches='tight')
        plt.close()
        ...
        with open(os.path.join(output_dir, "shap_significance_report.md"), 'w', encoding='utf-8') as f:
            ...
            if len(significant_features) > 0:
                f.write(f"在显著性水平α=0.05下，以下{len(significant_features)}个特征对C6H6吸附的影响具有统计学显著性：\n\n")
