...
# 加载数据
print("加载数据...")
df = pd.read_excel("F:\VS code\code for gas adsorption\gas adsorption-NO2.xlsx")
...
for name, model_results in results.items():
    ...
    plt.xlabel("Actual NO$_2$ Adsorption", fontsize=18)  # 增大X轴标签字体
    plt.ylabel("Predicted NO$_2$ Adsorption", fontsize=18)  # 增大Y轴标签字体
...
for i, (name, model_results) in enumerate(results.items(), 1):
    ...
    plt.xlabel("Actual NO$_2$ Adsorption", fontsize=16)  # 统一字体大小
    plt.ylabel("Predicted NO$_2$ Adsorption", fontsize=16)  # 统一字体大小
...
with open(os.path.join(output_dir, f'{best_model_name}_feature_importance_report.md'), 'w', encoding='utf-8') as f:
    f.write(f"# NO$_2$ Adsorption Feature Importance Analysis Report - {best_model_name} Model\n\n")
    ...
    f.write("\n## Analysis Conclusion\n")
    f.write("Based on multiple feature importance evaluation methods (built-in, permutation importance, SHAP values), the following features have the most significant impact on NO$_2$ adsorption performance:\n\n")
    ...
    f.write("\nThese features should be the focus when optimizing materials for NO$_2$ adsorption.\n")
