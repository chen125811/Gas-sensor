# -*- coding: utf-8 -*-
# xgboost_shap_importance_analysis.py - 使用XGBoost模型的SHAP分析计算变量重要性占比
...
# 设置输出目录 - 修改为XGBoost
output_dir = "F:\\VS code\\code for gas adsorption\\NO2\\各种树模型\\xgboost_shap_analysis"
os.makedirs(output_dir, exist_ok=True)

# 设置文件路径
data_path = "F:\\VS code\\code for gas adsorption\\gas adsorption-NO2.xlsx"
model_params_path = "F:\\VS code\\code for gas adsorption\\NO2\\各种树模型\\tree_models_results\\best_model_parameters.json"
...
try:
    ...
    if "XGBoost" in best_params:
        model_params = best_params["XGBoost"]
        print(f"使用XGBoost参数: {model_params}")
    else:
        raise ValueError("没有找到XGBoost参数")
    ...
    df = df.rename(columns={
        ...
        "Adsorption level vs Zrbpdc": "Adsorption_Level"
    })
    ...
    # 2. 清洗这些基本列
    df[basic_columns] = df[basic_columns].applymap(remove_asterisk)
    ...
    # 5. 转换为浮点数
    df[basic_columns] = df[basic_columns].astype(float)

    # 恢复原始特征列表
    selected_columns = basic_columns  # 不包含交互特征
    ...
    # 使用L1正则化的XGBoost
    params_converted['alpha'] = 0.1  # L1正则化参数
    params_converted['lambda'] = 1.0  # L2正则化参数
    
    # 6. 创建并训练XGBoost模型
    print("训练XGBoost模型...")
    model = xgb.XGBRegressor(**params_converted)
    ...
    print(f"XGBoost模型性能 - R²: {r2:.4f}, RMSE: {rmse:.4f}")
    ...
    with open(os.path.join(output_dir, "shap_importance_report.md"), 'w', encoding='utf-8') as f:
        ...
        # 修改这里，将C6H6改为NH3
        f.write("基于SHAP值分析，以下特征对NO2吸附贡献最大：\n\n")
        ...
        if top3_importance_sum > 50:
            f.write("表明它们是决定NO2吸附性能的关键因素。\n")
        else:
            f.write("表明它们是重要因素，但其他变量也有显著贡献。\n")
    
    #15. 计算SHAP值的统计显著性
    def calculate_shap_significance(model, X, feature_names, n_permutations=100, random_state=45):
    ...
    try:
        ...
        with open(os.path.join(output_dir, "shap_significance_report.md"), 'w', encoding='utf-8') as f:
            ...
            if len(significant_features) > 0:
                f.write(f"在显著性水平α=0.05下，以下{len(significant_features)}个特征对NO2吸附的影响具有统计学显著性：\n\n")
        ...
        # 在报告的结论部分添加关于交互特征的解释
        if 'Amino_X_BET' in significant_features:
            f.write("\n**氨基与BET比表面积的交互**表现出统计显著性，这表明氨基的效果取决于材料的比表面积大小。")
            f.write("较大比表面积的材料中，氨基基团可能提供更多的NO2吸附位点，从而产生协同效应。\n")
